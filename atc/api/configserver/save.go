...
func (s *Server) SaveConfig(w http.ResponseWriter, r *http.Request) {
	...
	case "application/json", "application/x-yaml":
		...
		if err != nil {
			s.handleBadRequest(w, fmt.Sprintf("yaml unmarshal failed: %s", err))
			return
		}
		...
		err = yaml.UnmarshalStrict(configWithoutUnknownToplevels, &config)
		if err != nil {
	...
	_, created, err := team.SavePipeline(pipelineName, config, version)
	if err != nil {
	...
	s.writeSaveConfigResponse(w, SaveConfigResponse{Warnings: warnings})
}
...
func (s *Server) writeSaveConfigResponse(w http.ResponseWriter, saveConfigResponse SaveConfigResponse) {
